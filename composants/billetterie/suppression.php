...
if(isset($_POST['ticket'])){
	...
	if($donnees['total'] != "1"){
		if(isset($_POST['attente'])){
			redirection('billetterie_reservations?errsuppression='.$_POST['ticket'].'&attente');
			exit();
		}else{
			redirection('billetterie_reservations?errsuppression='.$_POST['ticket']);
			exit();
		}
	}else{
		req('DELETE FROM '.$table.' WHERE id = "'.$_POST['ticket'].'" AND commentaires = "'.$_SESSION['utilisateur'].'"'); 
		redirection('billetterie_reservations?succsuppression='.$_POST['ticket']); 
		exit();
	}
}
?>
<center>
	<b>&Ecirc;tes-vous s&ucirc;r de vouloir supprimer votre ticket num&eacute;ro #<?php echo($_GET['ticket']);?> ?<br />
		<form action="billetterie_suppression" method="post">
...
<?php }?>
        	...
        	&nbsp;&nbsp;&nbsp;<a href="billetterie_reservations" class="btn btn-success">Annuler...</a>
		</form>
    </b>
</center>